...
const Browse = () => {
  ...
  useEffect(() => { supabase.from('subcategories').select('*').order('name').then(({ data }) => setSubs(data ?? [])); }, []);

  const activeCat = useMemo(() => cats.find((x) => x.slug === cat), [cats, cat]);
  const visibleSubs = useMemo(() => activeCat ? subs.filter((s) => s.category_id === activeCat.id) : [], [subs, activeCat]);
  ...
  useEffect(() => {
    (async () => {
      ...
      if (activeCat) qy = qy.eq('category_id', activeCat.id);
      if (sub && sub !== 'all') qy = qy.eq('subcategory_id', sub);
